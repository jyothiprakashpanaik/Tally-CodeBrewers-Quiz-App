...
def index(request):
    ...
    if request.user.is_authenticated:
        quiz_responses = QuizResponces.objects.filter(
            user_email=request.user.email).values("attempt_quiz").distinct()
...
def results(request, quiz_id):
    ...
    try:
        ...
        for question in questions:
            marks = QuizResponces.objects.get(
                Q(user_email=email) & Q(user_question=question)).marks
        ...
        context = {"score": score, "email": email, "username": username,
                   "quiz_name": quiz.title, "questions_report": questions_report}
...
def submit_answer(request, quiz_id, question_id):
    ...
    if request.method == "POST":
        ...
        user_options = [i.replace("option", "")
                        for i in request.POST if "option" in i]
        ...
        if(QuizResponces.objects.filter(user_email=user_email, user_question=user_question).exists()):
            messages.warning(
                request, "You cant change Your response of already submitted question")
        else:
            quiz_responce = QuizResponces.objects.create(
                user_email=user_email, attempt_quiz=quiz, user_question=user_question, marks=score)
            quiz_responce.save()

        exclude_questions = QuizResponces.objects.filter(
            user_email=user_email).values_list("user_question_id", flat=True)
        question = QuizQuestion.objects.filter(
            quiz_name=quiz).exclude(id__in=exclude_questions).first()
        ...
        messages.warning(
            request, "Access Denial! Please verify your email for further proceedings.")
        messages.info(
            request, "If you have submitted any question, it is stored in the server you can resume your test.")
...
def start_quiz(request, username, quiz_id):
    ...
    if len(QuizResponces.objects.filter(user_email=request.session["email"], attempt_quiz__id=quiz_id).all()) >= len(QuizQuestion.objects.filter(quiz_name__id=quiz_id)):
        messages.info(
            request, "Your response is stored can not reattempt the same quiz.")
        return redirect("/")
