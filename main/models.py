...
class QuizName(BaseModel):
    created_by = models.ForeignKey(
        User, related_name='quiz_created_by', on_delete=models.CASCADE)
    title = models.CharField(max_length=100, null=False, blank=False)
    description = models.TextField(default="this quiz was created on quizzApp")
    image = models.ImageField(
        upload_to="static/media/quiz_icon", default="quiz_icon/default.jpg")
...
class QuizQuestion(BaseModel):
    quiz_name = models.ForeignKey(
        QuizName, related_name='quiz_name', on_delete=models.CASCADE)
...
class QuizAnswer(BaseModel):
    question = models.ForeignKey(
        QuizQuestion, related_name='question', on_delete=models.CASCADE)
...
class QuizResponces(BaseModel):
    ...
    user_question = models.ForeignKey(
        QuizQuestion, on_delete=models.CASCADE, related_name="user_question_answer")
    attempt_quiz = models.ForeignKey(
        QuizName, on_delete=models.CASCADE, related_name="attempt_quiz", default=None)
