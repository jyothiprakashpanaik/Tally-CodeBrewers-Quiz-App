...
def new_quiz(request):
    ...
    if request.method == "POST":
        ...
        quiz = QuizName.objects.create(
            created_by=request.user, title=quiz_name)
...
def add_questions(request, qid):
    ...
    if request.method == "POST":
        ...
        question = QuizQuestion.objects.create(
            quiz_name=quiz, quiz_question=question, marks=marks)
        question.save()

        for option in options_list:
            print((option, correct_options))
            answer = QuizAnswer.objects.create(
                question=question, answer=request.POST[option], is_correct=(option in correct_options))
            answer.save()
...
def stats(request, qid):
    ...
    quiz_responce = quiz_responce.values('user_email').order_by(
        'user_email').annotate(Sum('marks')).values_list('marks__sum')
